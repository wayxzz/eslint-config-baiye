...
module.exports = {
    ...
    'parserOptions': {
        // ECMAScript 版本
        'ecmaVersion': 9,
        // 代码的类型： script(默认) | module(ECMAScript模块)
        'sourceType': 'script',
        // 额外的语言特性
        'ecmaFeatures': {
            // 启用jsx
    ...
    /**
     * 自定义解析器
     * ESLint 默认使用Espree作为其解析器, 下面的配置可以使用自定义的解析器
     */
    // 'parser': 'esprima',
    ...
    'rules': {
        ...
        // 禁止使用类似 eval() 的方法: 禁止在 setTimeout 或 setInterval 中传入字符串，如 setTimeout('alert('Hi!')', 100);
        ...
        // 强制对多行注释使用特定风格:  禁止使用连续的行注释来表示块注释。另外，要求块注释的每行之前有一个 *
        ...
        // 禁止在 import 和 export 和解构赋值时将引用重命名为相同的名字，比如 let { foo: foo } = bar;
